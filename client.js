...
function getdata(element,flag=false)
{
    const makediv = document.createElement("div");
        ...
        if(flag==true){
        listofconatiers.prepend(makediv);
        }
        else
        {
          listofconatiers.appendChild(makediv);
        }
...
document.addEventListener("DOMContentLoaded",function()
      {
        ...
        const sortElems = document.querySelectorAll('[id*=sort_by_]');

        sortElems.forEach((elm)=>{
          elm.addEventListener('click',function(e)
          {
            e.preventDefault();
            const sortby = this.querySelector("input");
            fetch(`http://localhost:7777/video-request?sortby=${sortby.value}`).then((bold)=>bold.json()).then((data)=>{
            listofconatiers.innerHTML = '';
